...
import { Save, Server, Mail, Shield, Users, AlertTriangle, Settings, Bot } from 'lucide-react';
...
interface SystemConfig {
    ...
    site_name: string;
    server_url: string;
    pseudo_streaming_enabled: boolean;
                ...
                <TabsContent value="model" className="mt-6 space-y-6">
                    <div className="bg-card border rounded-lg p-6 space-y-4">
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between">
                                <div>
                                    <Label htmlFor="pseudo_streaming_enabled">启用伪装流式</Label>
                                    <p className="text-sm text-muted-foreground">
                                        开启后，模型列表将额外提供带 "伪流/" 前缀的模型，这些模型将以伪流方式响应。
                                    </p>
                                </div>
                                <Switch
                                    id="pseudo_streaming_enabled"
                                    checked={config.pseudo_streaming_enabled}
                                    onCheckedChange={(checked) => setConfig({ ...config, pseudo_streaming_enabled: checked })}
                                />
                            </div>
                        </div>
                    </div>
